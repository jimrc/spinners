...
guessingGame <- function(){
  ...
  tt <- unix.time(
    for (i in 1:20) rect(12,.5,13,.55, col = "white", bord="white")
  )
  ...
  for(i in 1:20){
    ...
    Sys.sleep(napTime)
    rect(i*2.5-4, .7, 2.5*i+5.5, 1.1,  col = "white", bord="white")
  }
  ...
  text(x=c(23, 27),y=.2, c("L","R"),col=1:2, cex=3)

  answer <- samp2*0
  ## Loop to plot guess and evaluate it.
  for(i in 1:25){  ## row 1
    answer[i] <- identify(n=1, x=c(23,27), y=c(.2,.2), labels="", pos=TRUE)$ind
    text(c("L","R")[answer[i]], x=2*i-2, y=.6, cex=3, col=answer[i])
    if(answer[i] != samp2[i]) text("X" , x=2*i-2, y=.6, cex=3, col=samp2[i])
  }
  for(i in 25+1:25){  ## row 2
    answer[i] <- identify(n=1, x=c(23,27), y=c(.2,.2), labels="", pos=TRUE)$ind
    text(c("L","R")[answer[i]], x=2*(i-26), y=.4, cex=3, col=answer[i])
    if(answer[i] != samp2[i]) text("X" , x=2*(i-26), y=.4, cex=3, col=samp2[i])
  }
  ## show results
